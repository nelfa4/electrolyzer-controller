...
- Your example: MAX_CURRENT = 8A → SAVE_CURRENT = 5A (62.5%)
...
All files are in a single folder (no subfolders):
...
While the code has been tested for compilation and logical correctness, the user assumes all responsibility for testing and safe operation of their electrolyzer hardware. Electrolysis involves flammable gases (hydrogen), corrosive electrolytes (KOH/NaOH), and high currents — always follow proper safety protocols.
